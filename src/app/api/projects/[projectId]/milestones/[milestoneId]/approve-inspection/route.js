...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (milestone.admin_review !== 'submitted') {
            return NextResponse.json({ message: 'Milestone is not ready for approval.' }, { status: 400 });
        }
        ...
        const historyEntry = {
            ...
            editedBy: userName,
            changes: [{ field: 'Inspection', oldValue: 'Submitted for Approval', newValue: 'Approved' }],
        };
        milestone.editHistory = [historyEntry, ...(milestone.editHistory || [])];

        await writeData(projectData);
        return NextResponse.json({ message: 'Milestone approved and completed.', milestone });
    } catch (error) {
